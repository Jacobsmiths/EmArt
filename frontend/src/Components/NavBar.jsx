...
const NavBar = ({ navTabClass = "", navTabItems = [], className = "" }) => {
  return (
    <div className={`hidden md:flex justify-between items-center ${className}`}>
      <div className="flex items-center justify-center h-full relative">
        {navTabItems.map((link, index) => (
          <NavLink
            key={index}
            to={`/${link.toLowerCase()}`}
            className={`relative h-full flex items-center justify-center overflow-hidden group font-extrabold ${navTabClass}`}
          >
